...
class Session < ApplicationRecord
  ...
  def solicitor_name
    if people.first
      return people.first.custom_published_name if people.first.custom_published_name
      return people.first.full_name if people.first.full_name
    end
  end

  def solicitor_published_name
    if people.first
      person = people.first
      if person.custom_published_name
        "#{person.custom_published_name} (#{person.full_name})"
      else
        person.full_name
      end
    end
